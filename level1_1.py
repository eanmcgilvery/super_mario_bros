...
class Level1_1(Level):
    def __init__(self, settings, screen, enemies, objects, background, mario, ui, timers):
        ...
        # Warp zone for mario to go to sublevel
        self.pipe_warp1_x = self.settings.brick_lenth * 47 + self.settings.small_pipe_width / 2 - 2
        self.pipe_warp1 = pygame.Rect(self.settings.brick_lenth * 47 + self.settings.small_pipe_width / 2 - 2, settings.ground_level - settings.brick_lenth * 4 - 4, 4, 4)

        self.generate_map(settings, screen, objects, background, mario)
        self.enemy_spawn_triggers(enemies)
    ...
    def warp_triggers(self, mario_):
        if self.pipe_warp1.colliderect(mario_) and mario_.crouch:
            self.active = False
